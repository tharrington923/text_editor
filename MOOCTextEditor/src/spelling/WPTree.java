...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		WPTreeNode word1Root = new WPTreeNode(word1,null);
		visited.add(word1);
		queue.add(word1Root);
		...
		while(!queue.isEmpty()){
			curr = queue.remove(0);
			List<String> nearbyWords = nw.distanceOne(word1,true);
			for(String word : nearbyWords){
				if(!visited.contains(word)){
					curr.addChild(word);
					visited.add(word);
					wordNode = new WPTreeNode(word,curr);
					queue.add(wordNode);
					if(word == word2){
						LinkedList<String> path = (LinkedList<String>) wordNode.buildPathToRoot();
						return path;
					}
				}
			}
